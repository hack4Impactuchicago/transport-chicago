...
function scores_print($reviews, $submissions){
	...
	foreach($submissions as $sub){
		...
		$html_out .= "<tr>";
		$html_out .= "<td class=''><small>" . $aid  . "</small></td>";
		//$html_out .= "<td class=''><small>XXXXXX</small></td>";
		$html_out .= "<td class=''><small>" . $sub["author"]  . "</small></td>";
		$html_out .= "<td class=''><small>" . $sub["affiliation"]  . "</small></td>";
		$html_out .= "<td class='title'><small>" . $sub["title"]  . "</small></td>";
		if($found_rev != FALSE){
			// convert time to chicago time
			$chi_time = $found_rev['time'] . " UTC"; 
			...
			if(trim($found_rev['quality']) != -1){
				$html_out .= "<td>" . $found_rev['quality'] . "</td>";
				$html_out .= "<td>" . $found_rev['paramaters'] . "</td>";
				$html_out .= "<td>" . $found_rev['timely'] . "</td>";
				$html_out .= "<td>" . $found_rev['writing'] . "</td>";
				$html_out .= "<td>" . $found_rev['original'] . "</td>";
				$html_out .= "<td>" . $found_rev['credibility'] . "</td>";
			}else{
				$html_out .= "<td colspan='6'>You declined to review this abstract.</td>";
			}
			$html_out .= "<td><a href='tagging.php?art_id=" . $aid  . "&sid=". $found_rev['score_id'] ."'>Edit Review</a></td>";
		}else{
			$html_out .= "<td colspan='7'>You haven't reviewed this abstract.</td>";
			$html_out .= "<td><a href='tagging.php?art_id=$aid'>Go Review</a></td>";
		}
		$html_out .= "</tr>";
	}
	...
	if($html_out == ""){
		$html_out .= "<tr>";
		$html_out .= "<td colspan='4'>You haven't reviewed anything yet. Go <a href='tagging.php'>review</a> some.</td>";
		$html_out .= "</tr>";
	}
...
<div class="container">
	...
	<div class='table-responsive'>
		<table class="table table-striped table-bordered">
		  <tr>
		  	<th class=''>Abstract ID</th>
		  	<th class=''>Abstract Author</th>
